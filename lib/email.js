...
export async function notifyAssessmentComplete({ contact, scores }) {
  const to = getRegionalEmail() || "team@infoseck2k.com";
  const { overall, band, gaps, domains } = scores;

  const domainRows = domains
    .map((d) => `<tr><td style="padding:4px 12px;border:1px solid #e2e8f0">${d.name}</td><td style="padding:4px 12px;border:1px solid #e2e8f0;text-align:center">${d.score}%</td></tr>`)
    .join("");

  const gapList = gaps.map((g) => `<li>${g.name} (${g.score}%)</li>`).join("");

  return sendNotification({
    to,
    subject: `New IAM Assessment: ${contact.name} at ${contact.company} — ${band.label} (${overall}%)`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px">
        <h2 style="color:#00a46e">New Assessment Completed</h2>
        ...
        <h3>Maturity Score: ${overall}% — ${band.label}</h3>
        ...
        <h3>Top Gaps</h3>
        <ul>${gapList}</ul>
        <p style="margin-top:16px"><strong>Recommended service:</strong> ${band.recommendation}</p>
...
export async function notifyBookingRequest({ contact, expertise, region, availability, message }) {
